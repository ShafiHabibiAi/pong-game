use ggez::{ self, event, graphics, Context };
...
struct MainState {
    player_one_position: na::Point2<f32>,
    player_two_position: na::Point2<f32>,
...
impl MainState {
    pub fn new(ctx: &mut Context) -> Self {
        ...
        MainState {
            player_one_position: na::Point2::new(RACKET_WIDTH_HALF + PADDING, screen_h_half),
            player_two_position: na::Point2::new(
...
impl event::EventHandler for MainState {
    ...
    fn update(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        move_racket(&mut self.player_one_position, KeyCode::W, -1.0, ctx);
        move_racket(&mut self.player_one_position, KeyCode::S, 1.0, ctx);
        move_racket(&mut self.player_two_position, KeyCode::Up, -1.0, ctx);
        move_racket(&mut self.player_two_position, KeyCode::Down, 1.0, ctx);
        ...
        let intersects_player_1 =
            self.ball_pos.x - BALL_SIZE_HALF < self.player_one_position.x + RACKET_WIDTH_HALF &&
            self.ball_pos.x + BALL_SIZE_HALF > self.player_one_position.x - RACKET_WIDTH_HALF &&
            self.ball_pos.y - BALL_SIZE_HALF < self.player_one_position.y + RACKET_HEIGHT_HALF &&
            self.ball_pos.y + BALL_SIZE_HALF > self.player_one_position.y - RACKET_HEIGHT_HALF;
        ...
        let intersects_player_2 =
            self.ball_pos.x - BALL_SIZE_HALF < self.player_two_position.x + RACKET_WIDTH_HALF &&
            self.ball_pos.x + BALL_SIZE_HALF > self.player_two_position.x - RACKET_WIDTH_HALF &&
            self.ball_pos.y - BALL_SIZE_HALF < self.player_two_position.y + RACKET_HEIGHT_HALF &&
            self.ball_pos.y + BALL_SIZE_HALF > self.player_two_position.y - RACKET_HEIGHT_HALF;
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        draw_param.dest = self.player_one_position.into();
        graphics::draw(ctx, &racket_mesh, draw_param)?;

        draw_param.dest = self.player_two_position.into();
        graphics::draw(ctx, &racket_mesh, draw_param)?;
        ...
        let score_text = graphics::Text::new(
            format!("{}         {}", self.player_1_score, self.player_2_score)
        );
...
fn main() -> GameResult<()> {
    let context_builder: ContextBuilder = ggez::ContextBuilder::new("pong_0", "game");
    let (mut ctx, mut event_loop) = context_builder.build()?;
    graphics::set_window_title(&mut ctx, "pongy_0");
    let mut state = MainState::new(&mut ctx);
    let _ = event::run(&mut ctx, &mut event_loop, &mut state);
    Ok(())
}
